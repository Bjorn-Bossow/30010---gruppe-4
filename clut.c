...
int32_t sincalcc(float i) {
	...
	if (i < 0) {
		int tal = (512 + (i*512)/360);
		return expand(SIN[tal]);
	}
...
void rotateVector(float x, float y, float angle) {
	...
	int x1 = (vec.x*coscalcc(angle))-(vec.y*sincalcc(angle));
	int y1 = (vec.x*sincalcc (angle))+(vec.y*coscalcc(angle));
